...
export const Marketplace: React.FC = () => {
  ...
  const handleAnalyze = async (offer: Offer) => {
    if (!isAiConfigured()) {
        alert("La IA está cargando o falta la API Key.");
        return;
    }
    setAnalyzing(true);
    setShowAiModal(true);
    const result = await analyzeOffer(`Oferta: ${offer.title}. Precio: ${offer.price}`);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 pb-32">
      <div className="flex flex-col md:flex-row gap-4 mb-8 justify-between items-center">
        ...
        <div className="relative w-full md:max-w-xs">
          ...
          <input type="text" className="w-full pl-10 bg-slate-900 border border-slate-700 rounded-xl py-2 text-white outline-none focus:border-primary-500" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredOffers.map((offer) => (
          <div key={offer.id} className="bg-slate-800/80 rounded-2xl border border-white/10 overflow-hidden flex flex-col relative">
            {adminMode && (
            ...
            <div className="p-3 bg-slate-900/50 flex gap-2">
              <button onClick={() => handleAnalyze(offer)} className="flex-1 bg-slate-700 text-secondary-400 py-2.5 rounded-xl font-bold text-xs flex items-center justify-center gap-1"><SparklesIcon className="h-4 w-4" /> IA</button>
              <button onClick={() => setTradeOffer(offer)} className="flex-[2] bg-white text-slate-900 py-2.5 rounded-xl font-bold text-xs flex items-center justify-center gap-1 hover:bg-gray-100">Iniciar</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {showAiModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="relative bg-slate-800 rounded-2xl w-full max-w-md border border-white/10 shadow-2xl">
            ...
            <div className="p-6">
              {analyzing ? (
                <div className="flex flex-col items-center py-8"><ArrowPathIcon className="h-10 w-10 text-secondary-400 animate-spin" /><p className="text-secondary-300 mt-4">Analizando...</p></div>
              ) : (
                <div className="text-gray-200 text-sm"><ReactMarkdown>{aiAnalysis}</ReactMarkdown></div>
              )}
            </div>
          </div>
        </div>
      ...
      {/* MODAL INICIAR - FIJADO Z-INDEX Y VISIBILIDAD */}
      {tradeOffer && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80">
          <div className="relative bg-slate-800 rounded-3xl w-full max-w-sm border border-primary-500/30 overflow-hidden shadow-2xl">
            <div className="p-5 border-b border-white/5 flex justify-between items-center bg-slate-900">
              <h3 className="text-white font-bold">Operación P2P</h3>
            ...
            <div className="p-6 space-y-6">
              <div className="bg-slate-950 p-4 rounded-2xl border border-white/5 text-center">
                <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Usuario</p>
              ...
              <a href={getWhatsAppLink(tradeOffer) || '#'} target="_blank" rel="noreferrer" className="flex items-center justify-center w-full bg-green-600 text-white font-bold py-4 rounded-2xl shadow-lg active:scale-95 transition-transform">
                <ChatBubbleLeftRightIcon className="h-6 w-6 mr-2" /> WhatsApp del Vendedor
              </a>
              ...
              <div className="pt-2 space-y-3">
                <button onClick={() => { alert('¡Completado!'); setTradeOffer(null); }} className="w-full bg-primary-600 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2">
                  <CheckCircleIcon className="h-5 w-5" /> Marcar Completado
                </button>
                <button onClick={() => setTradeOffer(null)} className="w-full text-xs text-gray-500 font-bold py-2">Cancelar</button>
              </div>
            </div>
          </div>
        </div>
